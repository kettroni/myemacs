...
Adding this to fix weird bugs with elpy
#+begin_src emacs-lisp
...
(leaf restclient
  ...
  :require t)
** Scala
Basic scala-mode and sbt config.
#+begin_src emacs-lisp
(leaf scala-mode
  :ensure t
  :require t
...
(leaf lsp-mode
  :ensure t
  :require t
  ;; Optional - enable elisp-mode automatically in scala files
  :hook (scala-mode . elisp)
  :config (setq elisp-prefer-flymake nil))

;; Add company-elisp backend for metals
(leaf lsp-metals
  :ensure t
  :require t)
...
(leaf lsp-ui
  :ensure t
  :require t) ;; this is necessary to get info about compilation
#+end_src
